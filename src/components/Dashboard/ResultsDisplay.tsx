...
import { LineChart, BarChart, PieChart, ResponsiveContainer, Line, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Pie, Cell } from 'recharts';
...
const ResultsDisplay = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 w-full p-4">
        <div className="text-center max-w-xl">
          <h2 className="text-2xl font-semibold mb-4 text-analytics-primary">Welcome to GenAI Analytics</h2>
          <p className="text-gray-600 mb-6">Ask any question about your data using natural language. Our AI will analyze your query and provide relevant insights.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-analytics-light p-4 rounded-lg">
              <p className="text-analytics-primary font-medium">"Show me monthly revenue for the last year"</p>
            </div>
            <div className="bg-analytics-light p-4 rounded-lg">
              <p className="text-analytics-primary font-medium">"What were our top selling products last quarter?"</p>
            </div>
            <div className="bg-analytics-light p-4 rounded-lg">
              <p className="text-analytics-primary font-medium">"Compare sales performance across regions"</p>
            </div>
          </div>
        </div>
      </div>
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={results.data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="value" stroke="#6E59A5" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        );
      case 'bar':
  ...
  const renderInsights = () => {
    ...
    return (
      <div className="mt-4 bg-analytics-light p-4 rounded-lg">
        <h3 className="font-semibold text-analytics-primary mb-2">Key Insights</h3>
  ...
  return (
    <Card className="w-full shadow-md border-analytics-light">
      <CardHeader>
        <CardTitle className="text-2xl text-analytics-primary">{results.title}</CardTitle>
